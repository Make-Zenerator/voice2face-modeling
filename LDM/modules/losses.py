...
def get_loss_function(loss_function_str: str):
    ...
    if loss_function_str == 'perceptual_loss':

        return perceptual_loss

    elif loss_function_str == 'adversarial_loss':

        return adversarial_loss
    elif loss_function_str == 'total_loss':

        return total_loss

    elif loss_function_str == 'VQLPIPSWithDiscriminator':

        return VQLPIPSWithDiscriminator
...
def adversarial_loss(discriminator, real, fake):
    real_loss = F.binary_cross_entropy(discriminator(real), torch.ones_like(real))
    fake_loss = F.binary_cross_entropy(discriminator(fake), torch.zeros_like(fake))
    return (real_loss + fake_loss) / 2

def total_loss(model, real_images):
    reconstructed_images, _ = model(real_images)
    p_loss = perceptual_loss(model.vgg, real_images, reconstructed_images)
    d_loss = adversarial_loss(model.discriminator, real_images, reconstructed_images)
